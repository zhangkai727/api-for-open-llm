...
<<<<<<< HEAD
    ...
    if functions:
        ...
        )  # 调用OpenAI API的Qwen模型，并传入消息和函数
    else:
        response = client.chat.completions.create(model="Qwen", messages=messages)  # 调用OpenAI API的Qwen模型，并传入消息
    logger.info(response.model_dump())  # 记录响应的模型转储信息到日志
